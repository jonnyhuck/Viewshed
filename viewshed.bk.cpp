...
/**
 * Work out the distance between two locations
 */
// double doubleDistance(double fx, double fy, double tx, double ty) {
// 	return (double)sqrt(pow(fx - tx, 2) + pow(fy - ty, 2));
// }

/**
 * Get the bearing in degrees between two locations
 */
// double direction(int fx, int fy, int tx, int ty) {
// 	double d = fmod((90 - (atan2(ty - fy, tx - fx) / 3.14159265359 * 180) + 360), 360);
// 	printf("Direction: %G\n", d);
// 	return d;
// }
...
int main(int argc, char **argv) {
	...
	/* Now we have all of the settings, read in the data...*/
	...
	if (program_options.areUsingPointToPoint == true) {
	
		//printf("Using point to point view calculation mode\n");
		//printOptions(&program_options);
		...
		if (hDataset != NULL)
		{
			...
			//populate values for output data based upon the input
			output.pixelWidth = nXSize; //TODO: maybe change to width/hegiht
			output.pixelHeight = nYSize;
			output.pixelMinx = 0;
			...
			//TODO: The below should be removed to a function that returns true / false - this is what the Python will want

			//as we are in p2p mode, do a single p2p and print the result.
	...
	//TODO: Most of this is repeated and needs consolidating (the if and else contents)
	} else {	//in viewshed mode
		...
		if (hDataset != NULL)
		{
			...
			//get pixel-based data
			output.pixelWidth = (output.width) / program_options.resolution; //TODO: maybe change to width/hegiht
			output.pixelHeight = (output.height) / program_options.resolution;
			output.pixelMinx = 0;
			...
			GDALDriverH   hDriver = GDALGetDriverByName("GTiff"); //GTiff
			GDALDatasetH hDstDS;
